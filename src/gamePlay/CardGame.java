...
public class CardGame extends JFrame {
	...
	/* 게임타이틀, 카드버튼, 클로즈버튼, 타이머 생성 */
	static void gameUI(GameFrame gameframe) {
		...
		for(int i = 0; i < 16; i++) { //카드버튼의 뒷면 이미지 저장
			...
			cardbtn[i].addActionListener(gameframe);
		...
		gameframe.add(panelCenter);   //프레임에 카드판 추가
		//gameframe.add(panelNorth);  //프레임에 타이틀 패널 추가
		gameframe.add(pan); 		  //프레임에 바탕패널 추가
	}
	...
	/* 종료 팝업창 */
	static void Warning(GameFrame gameframe) {
